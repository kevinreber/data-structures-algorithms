...
class BinaryTree {
	...
	minDepth() {
		...
		function minDepthHelper(node) {
			...
			// if left node is empty check right side
			if (node.left === null) return minDepthHelper(node.right + 1);
			// if right node is empty check left side
	...
	nextLarger(lowerBound) {
		...
		while (queue.length) {
			...
			const higherThanLowerBound = currentVal > lowerBound;

			// boolean to reassign closest if currentVal is less than closest or closest is null
			const shouldReassignClosest = currentVal < closest || closest === null;
